    ...
    <div :class="[
      ...
      <div class="w-full px-2">
        <div class="w-full flex justify-between items-center sticky top-0 bg-white">
          <h1 class="text-xl ml-0.5">Orders</h1>
          <button 
        ...
        <TableView
          v-if="pageData?.orders"
          :columns="columns"
          :items="pageData?.orders.items"
          :links="pageData?.orders.links"
          :page="pageData?.orders.page"
          :perPage="pageData?.orders.per_page"
          :total="pageData?.orders.total"
          uniqueField="order_num"
          :orderBy="pageData?.orders.order_by"
          :order="pageData?.orders.order"
...
export default {
  ...
  data () {
    return {
      ...
      selected: [],
      columns: [
        {key: "order_num", label: "Order #", formatter: null},
        {key: "grand_total", label: "Grand Total", formatter: "currency"},
        {key: "payments_total", label: "Payments Total", formatter: "currency"},
        {key: "balance", label: "Balance", formatter: "currency"},
        {key: "num_payments", label: "# payments", formatter: null},
